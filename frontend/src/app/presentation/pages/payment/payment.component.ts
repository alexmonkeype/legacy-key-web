...
export class PaymentComponent implements OnInit {
  ...
  async pago() {
    ...
    const cuenta=this.wallterAddress;
    ...
    console.log("usdt");
    const Address = "0x62B34fDEa06121d537E02de1E871CBFf0124F790";
    window.web3 = await new Web3(window.ethereum);
    window.contract2 = await new window.web3.eth.Contract( ABI2, Address);
    console.log("paso por aqui");           
    const amount = await window.contract1.methods.amountPay().call(); 
    await window.contract2.methods.approve(SCLegacyKey,BigInt(amount).toString()).send({ from: cuenta });
    console.log("Approve terminado");
    await window.contract1.methods.payLegacy().send({ from: cuenta });
