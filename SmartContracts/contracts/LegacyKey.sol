...
contract LegacyKey is Ownable, ReentrancyGuard {
    ...
    function setAdLegacy(address _adLegacy) external onlyOwner {
        adLegacy = _adLegacy;
    }
    ...
    function setAmountPay(uint256 _amount) external onlyOwner {
        // PAGO UNICO
        amountPay = _amount * decUSDT;
    }

    function newMember(
        address[] memory _hijos,
        address[] memory _validador,
        uint256 _amount,
        uint256 _time
    ...
    function voteValidador(uint _count) external onlyWitnessPermit(_count) {
        ...
        require(
            deathWitness[msg.sender][_count].padre != address(0),
            "Wrong ID"
        );
    ...
    function withdrawHeir(uint _count) external onlyHeirPermit(_count) {
        ...
        require(
            father.sign > _heirs.div(2),
            "You still can't collect his inheritance"
        );
